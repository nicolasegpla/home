...
function FormCreateRoom() {
    ...
    const { closeModalRoom, handleDataChange, createElement, data } = useContext(GlobalContext);
        ...
        <>
            <div className="w-9/12 h-72 flex flex-col justify-center items-center bg-indigo-700 text-slate-200 rounded-md gap-6 mx-auto mt-40 drop-shadow-xl ">
                ...
                <div className="flex flex-col justify-center items-center text-center w-96 gap-4">
                    <input value={data.name} id="name" name="name" onChange={handleDataChange} className="w-56 h-12 rounded-md px-3 cursor-pointer text-slate-900" placeholder="kitchen" type="name" />
                    <button onClick={createElement} className="rounded-md w-56 h-12 bg-slate-100 text-indigo-600 font-semibold">Create room</button>
                </div>
            </div>
